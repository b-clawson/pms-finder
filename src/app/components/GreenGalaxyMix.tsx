...
export function GreenGalaxyMix() {
  ...
  const computeAmount = (materialAmount: number, totalParts: number) => {
  ...
  if (selectedMatch && (formulaDetail || formulaLoading || formulaError)) {
    const totalParts = formulaDetail
      ? formulaDetail.materials.reduce((sum, m) => sum + m.amount, 0)
      : 0;
          ...
          <div className="space-y-6">
            ...
            <div className="bg-white rounded-xl shadow-sm overflow-hidden">
              <div
                ...
                style={{ backgroundColor: formulaDetail.color.hex || selectedMatch.hex }}
              >
                <div className="text-white drop-shadow-md">
                  <h2 className="text-2xl font-bold">{formulaDetail.color.code}</h2>
                  <p className="text-sm opacity-90">{formulaDetail.color.name}</p>
                </div>
              </div>
              <div className="px-6 py-3 flex items-center gap-4 text-sm text-gray-500">
                <span className="font-mono">{formulaDetail.color.hex}</span>
                <span className="px-2 py-0.5 bg-gray-100 rounded text-xs">
                  {formulaDetail.color.category === 'UD' ? 'Uncoated Direct' : 'Coated Direct'}
                </span>
                {formulaDetail.comments && (
                  <span className="text-gray-400 italic">{formulaDetail.comments}</span>
                )}
              </div>
            </div>
            ...
            <div className="bg-white rounded-xl shadow-sm overflow-hidden">
              <div className="px-6 py-4 border-b border-gray-100">
                ...
                <p className="text-xs text-gray-500 mt-0.5">
                  {formulaDetail.materials.length} components
                </p>
              </div>
              <div className="overflow-x-auto">
                <table className="w-full text-sm">
                  ...
                  <tbody className="divide-y divide-gray-50">
                    {formulaDetail.materials.map((m, i) => {
                      const pct = totalParts > 0
                        ? ((m.amount / totalParts) * 100).toFixed(2)
                        : '0.00';
                      const amount = totalParts > 0
                        ? computeAmount(m.amount, totalParts)
                        : 0;
